...
const getById = async (req, res) => {
  ...
  if (!item) return res.status(404).json({ mensaje: 'cita no encontrado' });
  res.json(item);
};
...
const update = async (req, res) => {
  ...
  if (!actualizado) return res.status(404).json({ mensaje: 'cita no encontrado' });
  res.json(actualizado);
};

const eliminar = async (req, res) => {
  const eliminado = await service.deleteItem(req.params.id);
  if (!eliminado) return res.status(404).json({ mensaje: 'cita no encontrado' });
